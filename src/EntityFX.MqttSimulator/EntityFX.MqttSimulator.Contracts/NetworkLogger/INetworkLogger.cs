...
namespace EntityFX.MqttY.Contracts.NetworkLogger
{
    public interface INetworkLogger
    {
        ...
        void Push(long id, long tick, NetworkLoggerType type, string message, string protocol, string? category, NetworkLoggerScope? scope = null, int? ttl = null, int? queueLength = null);
        ...
        void Push(long tick, INetworkPacket packet, NetworkLoggerType type, string message, string protocol, string? category, NetworkLoggerScope? scope = null);

        void Push(long id, long tick, INode from, INode to, byte[]? packet, NetworkLoggerType type, string message, string protocol, string? category,
        ...
        void BeginScope(long tick, ref INetworkPacket packet, string scope);

        void EndScope<TContext>(long tick, ref NetworkPacket<TContext> packet);

        void EndScope(long tick, ref INetworkPacket packet);
